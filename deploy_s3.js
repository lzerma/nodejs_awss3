...
function uploadImages(path, country, subfolder) {

    if(subfolder) {
        var fileList = getFileList(path+subfolder);
    }
    else {
    ...
    fileList.forEach(function (entry) {
        if(entry.info.isFile()) {
            var fullpath = (subfolder) ? subfolder+entry.entry_path : entry.entry_path;
            uploadFile(country+'/'+fullpath, path+fullpath);
        }
        else {
            uploadImages(path, country, fullpath+'/')
        }
    });
}
...
function getFileList(path) {
    ...
    for (i = 0; i < filesFound.length; i++) {
        ...
        fileList.push({'entry_path': filesFound[i], 'info': fileInfo});
    }
